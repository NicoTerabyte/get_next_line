...
static char	*ft_readrest(int fd, char *res)
{
	char	*temp;
	int		bytes;
...
static char	*ft_writing(char *res)
{
	char	*line;
	int		i;
...
static char	*ft_new_static(char *res)
{
	char	*s;
	int		i;
	int		j;
	...
	if (!s)
		return (NULL);
	i++; //dobbiamo tenere conto che per aggiornare lo stash non teniamo conto del carattere \n
	j = 0;
	while(res[i])
	{
		s[j++] = res[i++];
